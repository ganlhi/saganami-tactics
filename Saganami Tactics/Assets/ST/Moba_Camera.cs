...
namespace ST
{
    ...
    {
        public bool overInteractableUi = false;
        ...
        private void Start()
        {
            ...
            if (settings.movement.useDefualtHeight && enabled)
            {
                ...
                (Input.GetButton(inputs.axis.button_rotate_camera)))
            {
                ...
                if (!settings.rotation.lockRotationX)
                {
                    ...
                    if (deltaMouseVertical != 0.0)
                    {
                        ...
                        {
                            if (deltaMouseVertical > deltaMouseDeadZone)
                            {
                                changeInRotationX = 1.0f;
                            }
                            else if (deltaMouseVertical < -deltaMouseDeadZone)
                            {
                                changeInRotationX = -1.0f;
                            }
                        }
                ...
                if (!settings.rotation.lockRotationY)
                {
                    ...
                    if (deltaMouseHorizontal != 0.0f)
                    {
                        if (settings.rotation.constRotationRate)
                        {
                            if (deltaMouseHorizontal > deltaMouseDeadZone)
                            {
                                changeInRotationY = 1.0f;
                            }
                            else if (deltaMouseHorizontal < -deltaMouseDeadZone)
                            {
                                changeInRotationY = -1.0f;
                            }
                        }
        ...
        private void CalculateCameraMovement()
        {
            ...
            else if (!overInteractableUi)
            {
                ...
                if ((inputs.useKeyCodeInputs) ?
                    (Input.GetKey(inputs.keycodes.CameraMoveUp)) :
                    (Input.GetButton(inputs.axis.button_camera_move_up)))
                {
                    movementVector += requirements.pivot.transform.up;
                }
                if ((inputs.useKeyCodeInputs) ?
                    (Input.GetKey(inputs.keycodes.CameraMoveDown)) :
                    (Input.GetButton(inputs.axis.button_camera_move_down)))
                {
                    movementVector -= requirements.pivot.transform.up;
                }
...
// Helper Classes
    ...
    public class Moba_Camera_KeyCodes
    {
        ...
        public KeyCode CameraMoveUp = KeyCode.PageUp;
        public KeyCode CameraMoveDown = KeyCode.PageDown;
    }
    ...
    public class Moba_Camera_Axis
    {
        ...
        public string button_camera_move_up = "Moba Camera Move Up";
        public string button_camera_move_down = "Moba Camera Move Down";
    }
